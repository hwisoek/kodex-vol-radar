...
def get_detailed_trading_strategy(
    ...
    if risk_score >= 85.0:
        if is_whipsaw_risk:
            if channel_pos > 70.0:
                ...
                return (
                    "🌪️ [단기 전략 03] 초고변동 진공 휩소 회피 (전면 관망)",
                    "#b91c1c",
            ...
            if channel_pos > 50.0 and is_strong_trend_up:
                return (
                    "🚀 [단기 전략 04] 모멘텀 돌파 스캘핑 (비중 축소/슬리피지 방어)",
                    "#ef4444",
                ...
                return (
                    "⚡ [단기 전략 05] 지지선 붕괴 하방 모멘텀 (포지션 스위칭)",
                    "#ef4444",
    ...
    # 2. 고변동성 추세/과열 (65.0 ~ 84.9)
        ...
        if is_whipsaw_risk:
            if rr_ratio > 1.2:
                ...
                return (
                    "🛑 [단기 전략 08] 가짜 돌파(Fakeout) 트랩 매도 대응",
                    "#ea580c",
    ...
    # 3. 중변동성 표준 국면 (40.0 ~ 64.9)
        ...
        if is_whipsaw_risk:
            if channel_pos > 50.0:
                return (
                    "🔄 [단기 전략 12] 박스 상단 수렴 후 페이크 역지정 매매",
                    "#0284c7",
                ...
                return (
                    "🎣 [단기 전략 13] 과매도 기반 쌍바닥 매수 (상위 프레임 체크)",
                    "#0284c7",
            ...
            if rr_ratio >= 1.25 and channel_pos <= 45.0:
                ...
                return (
                    "⚠️ [단기 전략 15] 손익비 열위 구간 진입 보류 및 비중 축소",
                    "#64748b",
                ...
                return (
                    "⏳ [단기 전략 16] 수렴 구간 브레이크아웃 대기 (방향성 탐색)",
                    "#0ea5e9",
                ...
                return (
                    "📈 [단기 전략 17] 표준 채널 내 지지/저항 핑퐁 트레이딩",
                    "#0ea5e9",
    ...
    # 4. 저변동성 안정 국면 (20.0 ~ 39.9)
        ...
        if is_whipsaw_risk:
            return (
                "🪤 [단기 전략 18-A] 박스권 내 비정형 속임수 파동 관망",
                "#059669",
            ...
            return (
                "💤 [단기 전략 20] 지루한 횡보장 스캘핑 자제 (수수료 방어)",
                "#10b981",
    ...
    # 5. 극저변동성 에너지 응축 (20.0 미만)
        ...
        if is_whipsaw_risk:
            ...
            return (
                "🛑 [단기 전략 24] 에너지 완충 구간 전면 관망 (휴식 권장)",
                "#059669",
...
def analyze_60d_macro_regime(h_data, current_price, trading_hours=6.5):
    ...
    if is_bull:
        if macro_pos >= 85.0:
            title = "🚀 [장기 전략 01] 중기 대세 상승 과열권 (추격 자제 & 분할 익절)"
            color = "#ef4444"
            desc = (
            ...
            title = "🌊 [장기 전략 02] 60일 정배열 눌림목 추세 추종 (바이앤홀드)"
            color = "#0ea5e9"
            desc = (
        ...
        if macro_pos <= 15.0:
            title = (
                "🕳️ [장기 전략 03] 60일 최저점 과매도 패닉 (역발상 분할 매집 준비)"
            )
            color = "#10b981"
            desc = (
            ...
            title = "⚡ [장기 전략 04] 중기 역배열 하락 지속 (현금 비중 극대화 / 숏 우위)"
            color = "#dc2626"
            desc = (
        ...
        if macro_pos >= 70.0:
            title = "🧱 [장기 전략 05] 60일 박스권 상단 저항 (비중 축소)"
            color = "#f59e0b"
            desc = (
            ...
            title = "📦 [장기 전략 06] 60일 박스권 하단 지지 (스윙 바닥 매수)"
            color = "#059669"
            desc = (
            ...
            title = "⏳ [장기 전략 07] 중기 수렴 지대 (에너지 응축/관망)"
            color = "#64748b"
            desc = (
