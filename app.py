...
with st.expander("🔬 [통계 및 실전 검증] FPCA 변동성 예측 모형 & 실전 동적 가이드 신뢰도", expanded=True):
    ...
    with tab_today:
        ...
        if len(today_data) > 0:
            ...
            # 종목별 합산 수익률의 평균 (장 전체 분위기 파악용)
            asset_total_pnls = [d["_sort_tot"] for d in today_data]
            avg_asset_pnl = float(np.mean(asset_total_pnls))
            ...
            t4.metric(
                "전체 평균 수익률",
                f"{avg_asset_pnl * 100:+.2f}%",
                delta="강세장" if avg_asset_pnl >= 0.005 else ("약세장" if avg_asset_pnl <= -0.005 else "횡보장"),
                delta_color="normal" if avg_asset_pnl >= 0 else "inverse"
            )
