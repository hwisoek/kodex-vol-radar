    ...
    try:
        ...
        try:
            ...
            if h_data is not None and "close" in h_data and len(h_data["close"]) >= 60:
                ...
                rolling_rv_thresholds = s_sigmas.rolling(window=48, min_periods=12).quantile(spike_quantile).bfill().values
                ...
                if is_3x:
                    ...
                    # 1x 우량주: 종목 자체 변동성 기준으로 파라미터 자동 결정
                    avg_sigma = float(np.median(pred_sigmas[-20:]))  # 최근 20봉 시그마 중간값
                    dip_rate = max(0.950, 1.0 - 2.5 * avg_sigma)    # 2.5σ 눌림 시 2차 매수
                    escape_target_pnl = max(0.004, 1.5 * avg_sigma)  # 1.5σ 달성 시 약익절 목표
                    hard_stop_rate = min(-0.060, -5.0 * avg_sigma)   # 5σ 손실 시 하드 손절 (최소 -6% 보장)
                    max_holding_bars = 48     # 우량주는 무거우므로 반등까지 충분한 시간(약 7~8일)을 줌
                    min_band_spread = 0.015
                    macro_allow_cap = 65.0

                # 1x 우량주는 자연 변동성이 작아 밴드가 너무 좁으므로 1.8배 확장
                # (레버리지/인버스는 자체 변동성이 충분해 그대로 유지)
                if is_3x or is_2x:
                    band_sigma = pred_sigmas
                else:
                    band_sigma = pred_sigmas * 1.8
                ...
                for i in range(60, len(h_prices)):
                    ...
                    if position is None:
                        ...
                        if is_calm and touched_lower and is_bullish_bounce and macro_allow and has_enough_spread:
                            ...
                            tp1_pnl = 0.0
                            # 매수 이유 기록
                            _entry_trend = "정배열(상승)" if is_bull else "비정배열(횡보)"
                            _entry_macro = f"채널{macro_pos:.0f}%"
                            entry_reason = f"{_entry_trend} | {_entry_macro} | 하단밴드반등"
                    ...
                    # 2) 보유 상태: 2차 분할 매수 및 익절/탈출/손절 관리
                        ...
                        if holding_units == 0.5 and not has_taken_tp1:
                            if curr_p <= first_entry_price * dip_rate:
                                if not is_3x and not is_2x:
                                    # ── [1x 전용] 1주 Lower Highs 계단형 하락 감지 ────────
                                    # 과거 1주(5거래일 × 거래시간)치 시간봉에서 로컬 고점을 추출,
                                    # 고점이 계속 낮아지는 패턴이면 계단형 하락으로 판단
                                    _n_week = int(round(5 * target_info.get("trading_hours", 6.5)))
                                    _w = h_prices[max(60, i - _n_week): i + 1]
                                    _highs = [
                                        _w[j] for j in range(1, len(_w) - 1)
                                        if _w[j] >= _w[j - 1] and _w[j] >= _w[j + 1]
                                    ]
                                    _is_lower_highs = (
                                        len(_highs) >= 2 and
                                        all(_highs[k] > _highs[k + 1] for k in range(len(_highs) - 1))
                                    )
                                else:
                                    # 2x / 3x: 패턴 체크 없이 바로 매수
                                    _is_lower_highs = False

                                if _is_lower_highs:
                                    # 🚨 계단형 하락 감지 → 2차 매수 포기, 1차분 즉시 청산
                                    _exit_pnl = (curr_p - first_entry_price) / first_entry_price
                                    net_final_pnl = _exit_pnl - fee_rate
                                    ...
                                    _si_trend = "정배열" if is_bull else "비정배열"
                                    _si_macro = f"채널{macro_pos:.0f}%"
                                    scale_in_reason = f"{_si_trend} | {_si_macro} | 눌림{_dip_pct:.1f}%"
                        ...
                        if is_trend_exit or is_escape_exit or is_spike or is_timeout or is_hard_stop:
                            ...
                            trade_log.append({
                                ...
                                "exit_reason": (
                                    "trend_exit" if is_trend_exit else
                                    "escape_exit" if is_escape_exit else
                                    "spike" if is_spike else
                                    "timeout" if is_timeout else
                                    "hard_stop" if is_hard_stop else "unknown"
                                ),
                            })
                            position = None
                ...
                if position == "LONG":
                    ...
                    current_holding = {
                        ...
                        "entry_reason": entry_reason,        # 1차 매수 진입 이유
                        "scale_in_reason": scale_in_reason,  # 2차 매수 진입 이유
                    }
        ...
        result_dict = {
            ...
            "recent_return": recent_return,  # 당일 등락률 (prices[0] 대비 현재가)
