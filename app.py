    ...
    try:
        ...
        try:
            ...
            if h_data is not None and "close" in h_data and len(h_data["close"]) >= 60:
                ...
                is_2x = any(kw in asset_name for kw in ["2배", "2X", "곱버스", "레버리지", "NVDL", "TSLL", "CONL", "MSTR", "마이크로스트래티지", "COIN", "코인베이스"])
                ...
                for i in range(60, len(h_prices)):
                    ...
                    # 🎯 계단식 하락 방어 (1주일 간 이평선이 꾸준히 하락 중이면 진입 금지)
                    ma20_prev_week = ma20_series[i - 30] if i >= 30 else c_ma20
                    is_staircase_drop = (c_ma20 < ma20_prev_week * 0.980) and (curr_p < c_ma20)
                    ...
                    if position is None:
                        ...
                        if is_real_bear or ma60_falling or is_staircase_drop:
                            macro_allow = False
                        elif is_bull:
                    ...
                    # 2) 보유 상태: 2차 분할 매수 및 익절/탈출/손절 관리
                        ...
                        # 상단 밴드 1차 분할 익절 (최소 +0.5% 수익 확보 시에만 tp1 인정 — 진입 직후 EWM 왜곡 방지)
                        if not has_taken_tp1 and (curr_p >= dyn_upper[i]) and current_pnl >= 0.005:
                            has_taken_tp1 = True
                            tp1_pnl = float(current_pnl)

                        # 청산 조건 분기
                        # 레버리지: mid_line 1봉 이탈 즉각 청산 / 우량주: 연속 2봉 이탈 확인 후 청산 (일시적 노이즈 방지)
                        is_trend_exit = has_taken_tp1 and (curr_p < mid_line[i]) and (
                            is_highly_volatile or prev_p < mid_line[i - 1]
                        )
                        
                        if is_highly_volatile:
                            # 레버리지/변동성 종목: 수익 나면 중심선 닿기만 해도 칼같이 방어적 탈출
                            is_escape_exit = (not has_taken_tp1) and (
                                current_pnl >= escape_target_pnl or curr_p >= mid_line[i]
                            )
                        else:
                            # 1배수 우량주 종목: 상단 밴드까지 여유롭게 랠리 홀딩
                            is_escape_exit = False
                            if not has_taken_tp1:
                                if holding_units > 0.5 and current_pnl >= escape_target_pnl:
                                    # 2차 추매까지 된 상태면 평단 방어를 위해 약수익 탈출
                                    is_escape_exit = True
                                elif current_pnl > 0 and curr_p < mid_line[i] and prev_p >= mid_line[i-1]:
                                    # 수익권에서 랠리를 타다가 '중심선을 하향 돌파'하며 꺾일 때만 안전마진 확보 익절
                                    is_escape_exit = True
                ...
                if trade_log:
                    ...
                    if loss_trades:
                        fig_long.add_trace(go.Scatter(
                            ...
                            customdata=[[t["pnl"] * 100, t.get("exit_reason", "?"), t.get("scale_in_time", "") or "없음"] for t in loss_trades],
