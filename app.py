            ...
            if h_data is not None and "close" in h_data and len(h_data["close"]) >= 60:
                ...
                for i in range(60, len(h_prices)):
                    ...
                    # 2) 보유 상태: 2차 분할 매수 및 익절/탈출/손절 관리
                        ...
                        # 배율별 눌림폭 충족 시 2차 매수
                        # 1x 우량주: 가격 눌림 + 하단밴드 터치 + 실질 약세장 아닐 때만 신중하게 진입
                        if holding_units == 0.5 and not has_taken_tp1:
                            price_dipped = curr_p <= first_entry_price * dip_rate
                            at_lower_band = curr_p <= dyn_lower[i]
                            scale_in_ok = price_dipped and (at_lower_band or (is_3x or is_2x)) and not is_real_bear
                            if scale_in_ok:
