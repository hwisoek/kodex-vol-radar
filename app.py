...
if all_events:
    ...
    for t in sorted(unique_times):
        ...
        portfolio_history.append({"time": time_str, "equity": mtm_value})
        ...
        if N_total >= 30:
            ...
            if "portfolio_history" in st.session_state and len(st.session_state["portfolio_history"]) > 0:
                    ...
                    for df in dfs:
                        ...
                        if first_v > 0:
                            ret_s = pd.Series(
                                ...
                                index=pd.to_datetime(d["utc_time"].values)
                            )
                ...
                fig = make_subplots(
                    ...
                    subplot_titles=(
                        "📈 포트폴리오 누적 수익률 (Cumulative Return %)",
                        "🌏 국장 / 🇺🇸 미장 평균 등락률 (%)"
                    )
                )
                ...
                fig.add_trace(
                    go.Scatter(
                        ...
                        hovertemplate="<b>%{x|%m/%d %H:%M}</b><br>누적: <b>%{y:+.3f}%</b><extra>포트폴리오</extra>"
                    ),
                    row=1, col=1
                )
                fig.add_hline(y=0, line_dash="dot",
                    line_color="rgba(148,163,184,0.5)", line_width=1, row=1, col=1)
                ...
                if not kr_ret.empty:
                    fig.add_trace(
                        go.Scatter(
                            ...
                            hovertemplate="<b>%{x|%m/%d %H:%M}</b><br>국장: <b>%{y:+.3f}%</b><extra>국장 평균</extra>"
                        ),
                        row=2, col=1
                    )
                # 미장 평균 등락률
                if not us_ret.empty:
                    fig.add_trace(
                        go.Scatter(
                            ...
                            hovertemplate="<b>%{x|%m/%d %H:%M}</b><br>미장: <b>%{y:+.3f}%</b><extra>미장 평균</extra>"
                        ),
                        row=2, col=1
                    )
                fig.add_hline(y=0, line_dash="dot",
                    line_color="rgba(148,163,184,0.4)", line_width=1, row=2, col=1)
                ...
                fig.update_layout(
                    ...
                    yaxis=dict(**_grid, ticksuffix="%", tickformat=".2f",
                               title=dict(text="누적 수익률", font=dict(size=11)), automargin=True),
                    yaxis2=dict(**_grid, ticksuffix="%", tickformat=".2f",
                                title=dict(text="시장 등락률", font=dict(size=11)), automargin=True),
                )
