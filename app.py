...
with st.expander("🔬 [통계 및 실전 검증] FPCA 변동성 예측 모형 & 실전 동적 가이드 신뢰도", expanded=True):
    ...
    with tab_today:
        ...
        if len(today_data) > 0:
            ...
            # 전체 종목 당일 평균 등락률 (장 전체 분위기 파악용 — 첤결 여부 무관하게 전 종목 대상)
            all_market_returns = [d.get("recent_return", 0.0) for d in full_ranked]
            market_avg_return = float(np.mean(all_market_returns)) if all_market_returns else 0.0
            ...
            t4.metric(
                "전 종목 평균 등락률",
                f"{market_avg_return * 100:+.2f}%",
                delta="강세장" if market_avg_return >= 0.005 else ("약세장" if market_avg_return <= -0.005 else "횡보장"),
                delta_color="normal" if market_avg_return >= 0 else "inverse"
            )
            t5.metric("최다 체결 종목", max(today_data, key=lambda x: x["_cnt"])["종목명"])
