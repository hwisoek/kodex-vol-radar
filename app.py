...
try:
    ...
    if len(prices) == 24:
        ...
        def get_detailed_trading_strategy(risk_score: float, channel_pos: float, rr_ratio: float, is_whipsaw_risk: bool, fpc_scores: list):
    fpc1, fpc2, fpc3 = fpc_scores[0], fpc_scores[1], fpc_scores[2]
    is_strong_trend_up = fpc1 > 0.01
    is_strong_trend_down = fpc1 < -0.01

    # 1. 극초고변동성 국면 (Risk Score >= 85)
    if risk_score >= 85.0:
        if is_whipsaw_risk:
            if channel_pos > 70.0:
                return (
                    ...
                    "#dc2626",
                    "극단적 과열 상태에서 휩소 징후가 포착되었습니다. 상단 돌파 시 추격 매수를 금지하고, 1분봉상 거래량 터진 음봉 확인 후 초단타 숏/매도 관점으로 대응하세요.",
                    "🚨 초고위험 (피크아웃)", "inverse"
                )
                ...
                return (
                    "💥 [전략 02] 패닉셀 투매 낙주 투입",
                    "#dc2626",
                    "극단적 패닉셀 투매 국면입니다. 손절선 이탈 시 일시적 언더슈팅 반등(데드캣)을 노린 분할 매수만 유효하며, 반등 실패 시 1틱 단위 칼손절이 필수입니다.",
                    "🚨 초고위험 (낙주)", "inverse"
                )
            else:
                return (
                    ...
                    "#b91c1c",
                    "호가 갭이 벌어지고 상하 변동폭이 극에 달했습니다. 슬리피지 비용이 통계를 압도하므로 신규 진입을 전면 중단하고 기존 잔고를 확정 청산하세요.",
                    "🚨 극위험 (관망 권장)", "inverse"
                )
        else:
            if channel_pos > 50.0 and is_strong_trend_up:
                return (
                    ...
                    "#ef4444",
                    "상승 관성이 극대화된 정방향 돌파 구간입니다. 전고점 돌파 호가창 체결 강도 유지 시 시장가 추격 진입 후 10~15틱 내외에서 즉시 분할 익절하세요.",
                    "🚨 고위험 (돌파)", "inverse"
                )
                ...
                return (
                    ...
                    "#ef4444",
                    "하방 변동성 폭발로 주요 지지 라인이 뚫리는 국면입니다. 롱 포지션은 즉시 전량 청산하고, 선물/해외 ETF의 경우 하방 추격 스캘핑이 유리합니다.",
                    "🚨 고위험 (하방돌파)", "inverse"
                )
            else:
                return (
                    ...
                    "#f97316",
                    "방향성은 중립이나 진폭 자체가 매우 큽니다. 상단선 도달 시 매도, 하단선 도달 시 매수하되 10분 이상 포지션을 홀딩하지 마세요.",
                    "🚨 고위험 (밴드터치)", "inverse"
                )

    # 2. 고변동성/강추세 국면 (65 <= Risk Score < 85)
        ...
        if is_whipsaw_risk:
            if rr_ratio > 1.2:
                return (
                    ...
                    "#ea580c",
                    "손익비는 유리하나 급격한 반전(휩소) 가능성이 큽니다. 채널 하단 근접 시 지정가로만 체결시키고, 목표가의 50% 지점에서 조기 익절하세요.",
                    "⚖️ 고위험 (단기역추세)", "inverse"
                )
            else:
                return (
                    ...
                    "#ea580c",
                    "전고점을 뚫는 척하다 꼬리를 달고 내려앉는 불트랩 확률이 높습니다. 돌파 확인 전 선진입을 금지하고 저항선 부근에서 물량을 정리하세요.",
                    "⚖️ 주의 (트랩 위험)", "inverse"
                )
        else:
            if channel_pos >= 60.0:
                return (
                    ...
                    "#0284c7",
                    "상승 추세가 단단하게 유지되고 있습니다. 5분봉 기준 5선/10선 지지를 확인하며 눌림목마다 분할 매수하고, 이평 이탈 시 즉시 정리하세요.",
                    "🔥 고변동 추세", "normal"
                )
                ...
                return (
                    ...
                    "#0284c7",
                    "안정적인 추세 파동 속 일시적 하단 터치입니다. 하단 지지선 체결 잔량 누적을 확인하고 기술적 V자 반등을 노려 진입하기 좋은 자리입니다.",
                    "🔥 매수 우위", "normal"
                )
            else:
                return (
                    ...
                    "#0ea5e9",
                    "채널 중간값에서 상방으로 방향을 틀기 시작했습니다. 손익비 균형 상태로 추세 추종 스윙/데이트레이딩 관점 홀딩이 유효합니다.",
                    "🔥 추세 지속", "normal"
                )

    # 3. 중변동성/균형 파동 국면 (40 <= Risk Score < 65)
        ...
        if is_whipsaw_risk:
            if channel_pos > 50.0:
                return (
                    ...
                    "#0284c7",
                    "중변동 구간에서 파동 비틀림이 감지되었습니다. 상단선 바로 아래에 익절을 걸어두고, 진입가 대비 본절 로스컷(Stop-loss)을 타이트하게 설정하세요.",
                    "⚖️ 보통 (비틀림)", "normal"
                )
            else:
                return (
                    ...
                    "#0284c7",
                    "하단선 지지 후 2차 저점 확인(쌍바닥 패턴) 구간입니다. 변동성이 크지 않아 슬리피지 위험이 낮으므로 분할 2회로 나누어 진입하세요.",
                    "⚖️ 보통 (눌림목)", "normal"
                )
        else:
            if rr_ratio >= 1.3:
                return (
                    ...
                    "#0ea5e9",
                    "하단선과 현재가의 거리가 가까워 손절폭은 극히 짧고 기대 수익폭은 큽니다. 리스크 대비 수익 효율이 가장 높은 최적의 진입 타점입니다.",
                    "✅ 적극 매수", "normal"
                )
            elif rr_ratio <= 0.7:
                return (
                    ...
                    "#64748b",
                    "상단 목표가에 이미 근접하여 추가 상승 룸 대비 하방 리스크가 큽니다. 신규 진입을 금지하고 보유 물량의 50% 이상을 현금화하세요.",
                    "⚖️ 보통 (익절 우선)", "normal"
                )
                ...
                return (
                    ...
                    "#0ea5e9",
                    "진폭이 압축되는 중간 지대입니다. 상방/하방 이탈 방향이 확인될 때까지 호가창을 관망하며 확인 매매로 대응하세요.",
                    "⚖️ 중립 (수렴)", "normal"
                )
            else:
                return (
                    ...
                    "#0ea5e9",
                    "규칙적인 파동을 그리는 전형적인 채널 장세입니다. 채널 하단 30% 매수, 상단 70% 분할 매도 규칙을 기계적으로 적용하세요.",
                    "⚖️ 보통 (채널)", "normal"
                )

    # 4. 저변동성/박스권 국면 (20 <= Risk Score < 40)
    elif risk_score >= 20.0:
        if channel_pos >= 75.0:
            return (
                "🧱 [전략 18] 박스권 천장 역매매 (숏/비중 축소)",
                "#10b981",
                "변동성 에너지가 고갈되어 상단선 돌파 에너지가 부족합니다. 박스권 천장 부근에서 전량 익절하거나 단기 되돌림을 노리는 숏이 유리합니다.",
                "🛡️ 안정 (박스상단)", "normal"
            )
            ...
            return (
                ...
                "#10b981",
                "하방 압력도 약해 바닥을 깰 확률이 낮습니다. 손절 기준선을 엄격히 걸고 하단 호가에 받쳐두는 지정가 매수가 유효합니다.",
                "🛡️ 안정 (박스하단)", "normal"
            )
        else:
            return (
                "💤 [전략 20] 지루한 횡보장 슬캘핑 자제 (수수료 주의)",
                "#10b981",
                "호가 틱 가치 대비 주가 변동폭이 너무 좁아 잦은 매매 시 거래세와 수수료로 시드가 잠식됩니다. 호가 갭 확대 전까지 매매 횟수를 줄이세요.",
                "🛡️ 안정 (횡보)", "normal"
            )

    # 5. 극저변동성/에너지 압축 국면 (Risk Score < 20)
    else:
        if is_whipsaw_risk:
            return (
                "🪤 [전략 21] 개미 털기용 잔파도 노이즈 무시",
                "#059669",
                "거래량이 말라붙은 상태에서 생기는 일시적 틱 튐 현상입니다. 기술적 지표 신뢰도가 떨어지므로 일체의 뇌동매매를 삼가세요.",
                "🛡️ 극안정 (노이즈)", "normal"
            )
            ...
            return (
                ...
                "#059669",
                "장기 횡보 후 상단선에 가격이 밀착되었습니다. 볼린저 밴드 스퀴즈 이후 상방 폭발 가능성을 열어두고 돌파 호가 확인 매수를 준비하세요.",
                "🔋 응축 (상방대기)", "normal"
            )
            ...
            return (
                ...
                "#059669",
                "거래량 없이 질질 흘러내리는 계단식 하락 패턴 위험이 있습니다. 저점이라 착각해 물타기를 하지 말고 바닥 거래량 폭증을 기다리세요.",
                "🛡️ 극안정 (하방주의)", "normal"
            )
        else:
            return (
                ...
                "#059669",
                "변동성이 최저 수준으로 수렴했습니다. 큰 추세가 분출되기 전 태풍의 눈 구간이므로 신규 포지션을 잡지 말고 휴식을 취하세요.",
                "🛡️ 극안정 (관망)", "normal"
            )
