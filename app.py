...
def analyze_60d_macro_regime(h_data, current_price, trading_hours=6.5, is_inverse=False):
    """
    is_inverse: 해당 종목이 인버스(하락 베팅) 상품인지 여부 (True/False)
    """
    ...
    # =========================================================
    # [A] 인버스(하락 베팅) 상품일 경우의 전략 (물타기 금지, 짧은 수익 실현 강조)
    # =========================================================
    if is_inverse:
        if is_bull: # 인버스 정배열 = 실제 시장은 대폭락 중
            ...
            if macro_pos >= 90.0:
                ...
                color = "#dc2626"
                desc = "시장이 극도의 공포에 빠져 인버스가 최고점입니다. 시장이 반등하면 수익이 순식간에 녹아내리니 당장 전량 익절하십시오."
                action = "전량 익절 필수"
            elif macro_pos >= 80.0:
                title = "🔥 [인버스 02] 하락장 과열 구간 (인버스 분할 익절)"
                color = "#ef4444"
                desc = "시장 하락이 가속화되어 수익이 크게 났습니다. 더 떨어지길 기도하지 마시고 욕심 없이 분할로 이익을 챙기십시오."
                action = "분할 익절"
            elif macro_pos >= 65.0:
                title = "📉 [인버스 03] 시장 패닉셀 진행 중 (짧게 보유)"
                color = "#0ea5e9"
                desc = "시장 폭락으로 인버스 수익이 커지는 구간입니다. 단, 길게 끌고 가지 마시고 방망이를 짧게 잡으십시오."
                action = "단기 보유"
            elif macro_pos >= 50.0:
                title = "⚠️ [인버스 04] 인버스 허리 구간 (수익 보존 주의)"
                color = "#38bdf8"
                desc = "하락장이 이어지고 있으나 언제든 기술적 반등이 나올 수 있습니다. 수익금을 지키는 데 집중하십시오."
                action = "수익 방어"
            elif macro_pos >= 35.0:
                title = "⏳ [인버스 05] 시장 하락 잠시 멈춤 (신규 진입 자제)"
                color = "#94a3b8"
                desc = "시장이 숨을 고르고 있습니다. 방향이 바뀔 수 있으니 섣부른 인버스 추가 매수는 자제하십시오."
                action = "관망"
            elif macro_pos >= 20.0:
                title = "🎯 [인버스 06] 시장 단기 반등 (짧은 타점 노리기)"
                color = "#10b981"
                desc = "시장이 살짝 반등하여 인버스가 싸졌습니다. 손절을 아주 빡빡하게 잡고 짧은 단타로만 접근하십시오."
                action = "단타 진입"
            elif macro_pos >= 10.0:
                title = "⚡ [인버스 07] 시장 강한 반등 (손절선 엄수)"
                color = "#059669"
                desc = "시장이 꽤 강하게 반등 중입니다. 물렸다면 기도하지 마시고 기계적인 칼손절을 준비하십시오."
                action = "칼손절 대기"
            else:
                title = "🧨 [인버스 08] 인버스 추세 붕괴 위험 (즉각 탈출)"
                color = "#b91c1c"
                desc = "시장이 상승으로 완전히 돌아설 위험이 큽니다. 인버스 상승 추세가 깨졌으니 즉시 탈출하십시오."
                action = "즉시 손절"
            ...
            if macro_pos >= 90.0:
                ...
                color = "#dc2626"
                desc = "대세 상승장 중에 기적적으로 시장이 꺾였습니다. 운이 좋았습니다. 미련 없이 인버스를 전량 팔고 빠져나오십시오."
                action = "전량 매도"
            elif macro_pos >= 80.0:
                title = "🩹 [인버스 10] 시장 단기 조정 (비중 축소)"
                color = "#ef4444"
                desc = "시장이 잠시 주춤하여 인버스가 조금 올랐습니다. 물려있던 인버스 물량을 줄일 수 있는 마지막 기회입니다."
                action = "비중 축소"
            elif macro_pos >= 65.0:
                title = "🪤 [인버스 11] 가짜 하락장 경계 (현금 확보)"
                color = "#f59e0b"
                desc = "시장이 하락하는 척하지만 다시 강하게 오를 확률이 높습니다. 속지 마시고 인버스를 팔아 현금을 챙기십시오."
                action = "현금 확보"
            elif macro_pos >= 50.0:
                title = "🛑 [인버스 12] 대세 상승장 지속 (진입 절대 금지)"
                color = "#64748b"
                desc = "시장이 꾸준히 오르고 있습니다. 곧 떨어질 것이라 함부로 예측하여 인버스를 사지 마십시오."
                action = "매수 금지"
            elif macro_pos >= 35.0:
                title = "📉 [인버스 13] 인버스 가치 녹는 중 (물타기 금지)"
                color = "#64748b"
                desc = "시장이 오르면서 인버스의 가치가 계단식으로 녹아내리고 있습니다. 절대 물타기 하지 마십시오."
                action = "물타기 금지"
            elif macro_pos >= 20.0:
                title = "🕳️ [인버스 14] 인버스 계좌 주의보 (접근 금지)"
                color = "#0ea5e9"
                desc = "시간이 지날수록 인버스는 구조적으로 돈이 녹습니다. 가격이 많이 떨어졌다고 절대 사면 안 됩니다."
                action = "매수 절대 금지"
            elif macro_pos >= 10.0:
                title = "🚨 [인버스 15] 강력한 대세 상승장 (시장 순응)"
                color = "#10b981"
                desc = "시장이 끝없이 오르고 있습니다. 인버스는 쳐다보지도 마시고, 차라리 시장 상승에 베팅하십시오."
                action = "접근 금지"
            else:
                title = "☠️ [인버스 16] 인버스 지하실 파산 위험 (칼손절)"
                color = "#059669"
                desc = "언젠간 시장이 떨어지겠지라는 생각으로 버티면 계좌가 파산합니다. 지금이라도 기계적으로 손절하십시오."
                action = "칼손절 필수"
            ...
            if macro_pos >= 90.0:
                ...
                color = "#dc2626"
                desc = "시장이 지루하게 횡보 중이며 인버스가 고점입니다. 더 오르길 바라지 말고 무조건 파십시오."
                action = "천장 매도"
            elif macro_pos >= 80.0:
                title = "💰 [인버스 18] 횡보장 단기 하락 (이익 실현)"
                color = "#f59e0b"
                desc = "시장이 살짝 내려 인버스가 수익권입니다. 금방 다시 시장이 오를 수 있으니 안전하게 이익을 챙기십시오."
                action = "분할 익절"
            elif macro_pos >= 65.0:
                title = "⚖️ [인버스 19] 시장 방향성 탐색 중 (비중 축소)"
                color = "#fbd38d"
                desc = "시장이 위아래 눈치를 보고 있습니다. 인버스 비중을 절반으로 줄이고 리스크를 관리하십시오."
                action = "비중 축소"
            elif macro_pos >= 50.0:
                title = "😑 [인버스 20] 방향 없는 시장 (매매 보류)"
                color = "#94a3b8"
                desc = "시장에 뚜렷한 방향이 없습니다. 잦은 인버스 매매는 수수료만 낭비될 수 있으니 지켜만 보십시오."
                action = "관망"
            elif macro_pos >= 35.0:
                title = "🥱 [인버스 21] 횡보장 지속 (휴식 권장)"
                color = "#94a3b8"
                desc = "지루한 장세가 이어집니다. 인버스는 횡보장에서도 가치가 깎일 수 있으니 매매를 쉬십시오."
                action = "관망"
            elif macro_pos >= 20.0:
                title = "⏱️ [인버스 22] 횡보장 단기 헷징 (짧은 매수)"
                color = "#34d399"
                desc = "시장이 박스권 상단에 닿아 다시 떨어질 수 있는 자리입니다. 보험용으로 아주 짧게 단타만 노려보십시오."
                action = "짧은 단타"
            elif macro_pos >= 10.0:
                title = "🎁 [인버스 23] 횡보장 하단 헷징 (손절선 엄수)"
                color = "#10b981"
                desc = "시장이 천장을 찍어 인버스가 바닥에 왔습니다. 짧게 사보되, 예상과 다르면 바로 손절하십시오."
                action = "단기 헷지"
            else:
                title = "🧨 [인버스 24] 박스권 상단 돌파 위험 (즉시 손절)"
                color = "#b91c1c"
                desc = "시장이 박스권을 뚫고 위로 날아갈 조짐이 보입니다. 뒤도 돌아보지 말고 인버스를 손절하십시오."
                action = "칼손절 대기"

    # =========================================================
    # [B] 일반 종목 (주식, 상승형 ETF) - 기존 로직 유지
    # =========================================================
    else:
        if is_bull:
            trend = "상승 추세"
            if macro_pos >= 90.0:
                title = "🚀 [장기 전략 01] 상승장 천장 도착 (전량 익절 준비)"
                color = "#dc2626"
                desc = "더 이상 오르기 힘든 꼭대기입니다. 욕심부리지 말고 모두 팔 준비를 하십시오."
                action = "전량 익절 대기"
            elif macro_pos >= 80.0:
                title = "🔥 [장기 전략 02] 상승장 과열 구간 (분할 매도 시작)"
                color = "#ef4444"
                desc = "이미 많이 올랐습니다. 갖고 있는 것을 조금씩 팔아서 이익을 챙기시길 바랍니다."
                action = "분할 익절"
            elif macro_pos >= 65.0:
                ...
                color = "#10b981"
                desc = "오르다가 잠시 가격이 싸졌습니다. 조금 더 사모으기 좋은 기회입니다."
                action = "눌림목 매수"
            elif macro_pos >= 10.0:
                title = "💎 [장기 전략 07] 상승장 깊은 눌림 (바닥 줍기)"
                color = "#059669"
                desc = "상승장인데 바닥까지 깊게 내려왔습니다. 과감하게 주워 담아도 좋은 자리입니다."
                action = "적극 매수"
            else:
                title = "🚨 [장기 전략 08] 상승장 붕괴 경계 (손절 대기)"
                color = "#b91c1c"
                desc = "튼튼하던 지지선이 깨질 위험이 있습니다. 상승 기대감을 버리고 도망칠 준비를 하십시오."
                action = "손절 대기"
            ...
            if macro_pos >= 90.0:
                ...
                color = "#dc2626"
                desc = "떨어지던 중에 웬일로 꼭대기까지 올랐습니다. 미련 없이 다 팔고 도망치십시오."
                action = "전량 매도"
            elif macro_pos >= 80.0:
                title = "🩹 [장기 전략 10] 하락장 단기 반등 (분할 손절)"
                color = "#ef4444"
                desc = "잠깐 반짝 오르는 중입니다. 이때를 틈타 물려있는 것을 조금씩 손절하십시오."
                action = "비중 축소"
            elif macro_pos >= 65.0:
                title = "🪤 [장기 전략 11] 가짜 상승 경계 (현금 확보)"
                color = "#f59e0b"
                desc = "오르는 척하지만 다시 떨어질 확률이 높습니다. 속지 말고 현금을 챙겨두시길 바랍니다."
                action = "현금 확보"
            elif macro_pos >= 50.0:
                title = "🛑 [장기 전략 12] 하락장 중간 저항 (추격 매수 금지)"
                color = "#64748b"
                desc = "하락하는 힘이 강해서 계속 떨어질 수 있습니다. 절대 따라 사지 마십시오."
                action = "매수 금지"
            elif macro_pos >= 35.0:
                title = "📉 [장기 전략 13] 하락 가속 구간 (관망)"
                color = "#64748b"
                desc = "떨어지는 속도가 붙고 있습니다. 절대 사지 말고 꾹 참고 지켜만 보십시오."
                action = "관망"
            elif macro_pos >= 20.0:
                title = "🕳️ [장기 전략 14] 공포의 지하실 (매수 금지)"
                color = "#0ea5e9"
                desc = "계속 떨어지며 공포심이 커지는 구간입니다. 아직 바닥이 아니니 사면 안 됩니다."
                action = "매수 금지"
            elif macro_pos >= 10.0:
                title = "🔎 [장기 전략 15] 찐바닥 근접 (매수 준비)"
                color = "#10b981"
                desc = "거의 다 떨어졌습니다. 지금 당장 사지는 마시고, 슬슬 사볼 준비만 해보십시오."
                action = "매수 대기"
            else:
                title = "🦸‍♂️ [장기 전략 16] 극단적 패닉셀 (용기 내서 매집)"
                color = "#059669"
                desc = "모두가 공포에 질려 내던지고 있습니다. 용기를 내어 조금씩 주워 담아 볼 만합니다."
                action = "분할 매집"
            ...
            if macro_pos >= 90.0:
                ...
                color = "#dc2626"
                desc = "일정한 상자 안에 갇혀있는데 천장에 닿았습니다. 뚫기 어려우니 무조건 파십시오."
                action = "천장 매도"
            elif macro_pos >= 80.0:
                title = "💰 [장기 전략 18] 박스권 상단 저항 (이익 실현)"
                color = "#f59e0b"
                desc = "상자 위쪽에 다가왔습니다. 욕심내지 말고 안전하게 이익을 챙겨두십시오."
                action = "분할 익절"
            elif macro_pos >= 65.0:
                title = "⚖️ [장기 전략 19] 상향 돌파 시도 (절반 매도)"
                color = "#fbd38d"
                desc = "위로 뚫고 나갈지 고민하는 자리입니다. 혹시 모르니 절반만 안전하게 파십시오."
                action = "비중 축소"
            elif macro_pos >= 50.0:
                ...
                color = "#94a3b8"
                desc = "상자 한가운데서 살짝 아래입니다. 확실한 방향이 정해질 때까지 푹 쉬십시오."
                action = "관망"
            elif macro_pos >= 20.0:
                title = "🧺 [장기 전략 22] 박스권 하단 지지 (분할 매수)"
                color = "#34d399"
                desc = "상자 밑바닥에 가까워졌습니다. 슬슬 사모아 볼 만한 안전한 자리입니다."
                action = "하단 매수"
            elif macro_pos >= 10.0:
                title = "🎁 [장기 전략 23] 박스권 바닥 줍기 (적극 매수)"
                color = "#10b981"
                desc = "상자 밑바닥에 딱 닿았습니다. 싸게 살 수 있는 아주 좋은 기회이니 매수하십시오."
                action = "적극 매수"
            else:
                title = "🧨 [장기 전략 24] 박스권 하향 이탈 (칼손절)"
                color = "#b91c1c"
                desc = "튼튼하던 바닥이 뚫려버렸습니다. 지하로 끝없이 추락할 수 있으니 바로 도망치십시오."
                action = "손절 대기"
