    ...
    with tab_simulation:
        ...
        if N_total >= 30:
            ...
            if "portfolio_history" in st.session_state and len(st.session_state["portfolio_history"]) > 0:
                ...
                drawdown = df_eq["equity"] / roll_max - 1.0
                mdd = float(drawdown.min())
                final_return = float(df_eq["equity"].iloc[-1] - 1.0)
                
                e1, e2, e3 = st.columns(3)
                e1.metric("💰 MTM 누적 수익률", f"{final_return*100:+.2f}%")
                e2.metric("📉 최대 낙폭 (MDD)", f"{mdd*100:.2f}%")
                e3.metric("🔄 총 포지션 체결/청산", f"{N_total:,}회")
                
                st.markdown("##### 📈 포트폴리오 MTM 자산 성장 곡선 (Equity Curve)")
                st.line_chart(df_eq["equity"], height=250)
