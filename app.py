    ...
    with tab:
        ...
        with col_long:
            ...
            if macro is not None:
                ...
                future_labels_swing = [last_h_time]
                future_upper_swing = [current_price]
                future_lower_swing = [current_price]

                dummy_counter = 0
                for d_idx in range(1, 6):
                    # 예측 구간 시각적 확장을 위한 투명 패딩 추가 (각 일자별 18칸)
                    for _ in range(18):
                        dummy_counter += 1
                        future_labels_swing.append("\u200b" * dummy_counter)
                        future_upper_swing.append(None)
                        future_lower_swing.append(None)

                    scale = np.sqrt(d_idx / 5.0)
                    d_drift = macro["drift_5d"] * (d_idx / 5.0)
                    d_range = macro["range_5d"] * scale
                    future_labels_swing.append(f"D+{d_idx}")
                ...
                fig_long.add_trace(go.Scatter(
                    ...
                    line=dict(color="rgba(16,185,129,0.6)", width=1.5, dash="dot"), connectgaps=True
                ))
                fig_long.add_trace(go.Scatter(
                    ...
                    fill="tonexty", fillcolor="rgba(16,185,129,0.06)", connectgaps=True
                ))

                # 4. 미래 실행 기준선 (빈 갭을 무시하도록 배열화)
                tp1_line_y = [tp1_target if y is not None else None for y in future_upper_swing]
                buy2_line_y = [buy2_price if y is not None else None for y in future_upper_swing]
                tp1_line_y[0] = tp1_target
                buy2_line_y[0] = buy2_price

                fig_long.add_trace(go.Scatter(
                    x=future_labels_swing, y=tp1_line_y, mode="lines",
                    name="1차 50% 분할 익절선", line=dict(color="#dc2626", width=1.8, dash="dash"), connectgaps=True
                ))
                fig_long.add_trace(go.Scatter(
                    x=future_labels_swing, y=buy2_line_y, mode="lines",
                    name="2차 눌림 매수선 (-1%)", line=dict(color="#047857", width=1.8, dash="dash"), connectgaps=True
                ))
