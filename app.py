...
def fetch_recent_1h_candles(symbol: str):
    try:
        ...
        if df_1h is not None and not df_1h.empty and "Close" in df_1h.columns:
            df_clean = df_1h.dropna(subset=["Open", "Close", "High", "Low"])
            if len(df_clean) >= 60:
                idx_utc = df_clean.index.tz_convert('UTC') if df_clean.index.tz is not None else df_clean.index.tz_localize('UTC')
                return {
                    "open": df_clean["Open"].values.astype(float),
    ...
    try:
        ...
        try:
            ...
            if h_data is not None and "close" in h_data and len(h_data["close"]) >= 60:
                ...
                is_2x = any(kw in asset_name for kw in ["2배", "2X", "곱버스", "레버리지", "NVDL", "TSLL", "CONL"])
                
                # 1x 우량주는 스파이크 민감도를 낮추기 위해 90분위수 사용 (레버리지는 80분위수 유지)
                spike_quantile = 0.80 if (is_3x or is_2x) else 0.90
                # bfill() 제거 (초기 NaN 유지)
                rolling_rv_thresholds = s_sigmas.rolling(window=48, min_periods=12).quantile(spike_quantile).values

                mid_line = s_prices.ewm(span=10).mean().values
                
                # 1x 우량주 밴드 확장 (1.8 -> 2.5)
                if is_3x or is_2x:
                    band_sigma = pred_sigmas
                else:
                    band_sigma = pred_sigmas * 2.5
                dyn_lower = mid_line * (1.0 - band_sigma)
                dyn_upper = mid_line * (1.0 + band_sigma)
                ...
                c_spread = np.maximum(rolling_high - rolling_low, 1e-5)
                macro_pos = np.clip(((h_prices - rolling_low) / c_spread) * 100.0, 0.0, 100.0)

                # GC_REGIME: 진입 시점의 종목 자체 360봉 채널 위치 (0~100)
                # 현재 봉까지의 정보만 사용하므로 신호 시점 기준 룩어헤드 없음.
                rolling_high_360 = s_prices.rolling(360, min_periods=360).max().values
                rolling_low_360 = s_prices.rolling(360, min_periods=360).min().values
                macro_spread_360 = np.maximum(rolling_high_360 - rolling_low_360, 1e-5)
                signal_macro_pos = np.clip(
                    ((h_prices - rolling_low_360) / macro_spread_360) * 100.0,
                    0.0,
                    100.0,
                )

                df_bt = pd.DataFrame({
                    "utc_time": h_data["utc_times"],
                    "time_str": h_data["times"],
                    "open": h_data["open"],
                    "close": h_prices,
                    "high": h_data["high"],
                    "low": h_data["low"],
                    "curr_sigma": pred_sigmas,
                    "rv_threshold": rolling_rv_thresholds,
                    "mid_line": mid_line,
                    "dyn_upper": dyn_upper,
                    "dyn_lower": dyn_lower,
                    "c_ma20": ma20_series,
                    "c_ma60": ma60_series,
                    "macro_pos": macro_pos,
                    "signal_macro_pos": signal_macro_pos,
                    "is_3x": is_3x,
                    "is_2x": is_2x,
                    "is_kr": target_info["is_kr"],
...
# ==============================================================================
# 🎯 포트폴리오 통합 시뮬레이션 (최대 10슬롯)
# ==============================================================================
import collections
import pandas as pd

def build_market_features(all_res, target_symbol):
    df_m = None
    for res in all_res:
        if res.get("df_bt") is not None and res["target_info"]["symbol"] == target_symbol:
            df_m = res["df_bt"].copy()
            break
    if df_m is None or df_m.empty:
        return None
    
    df_m["ma60_prev5"] = df_m["c_ma60"].shift(5).fillna(df_m["c_ma60"])
    df_m["ma60_falling"] = df_m["c_ma60"] < (df_m["ma60_prev5"] * 0.998)
    df_m["is_bull"] = df_m["close"] > df_m["c_ma20"]
    
    m_feat = df_m[["utc_time", "is_bull", "ma60_falling", "macro_pos"]].copy()
    m_feat = m_feat.sort_values("utc_time")
    
    # 🎯 SPY / KODEX200 레짐은 shift(1) - 1봉 지연
    m_feat["is_bull_m"] = m_feat["is_bull"].shift(1)
    m_feat["ma60_falling_m"] = m_feat["ma60_falling"].shift(1)
    m_feat["macro_pos_m"] = m_feat["macro_pos"].shift(1)
    return m_feat.dropna(subset=["is_bull_m"])

spy_feat = build_market_features(all_calculated, "SPY")
kodex_feat = build_market_features(all_calculated, "069500.KS")

all_events = []
for res in all_calculated:
    df_bt = res.get("df_bt")
    if df_bt is None or df_bt.empty:
        continue
        
    df_bt = df_bt.sort_values("utc_time").copy()
    m_feat = kodex_feat if res["target_info"]["is_kr"] else spy_feat
    
    if m_feat is not None and not m_feat.empty:
        df_bt = pd.merge_asof(df_bt, m_feat[["utc_time", "is_bull_m", "ma60_falling_m", "macro_pos_m"]], on="utc_time", direction="backward")
    else:
        # 지수 데이터가 없으면 자체 레짐 사용하되 shift(1)
        df_bt["ma60_prev5"] = df_bt["c_ma60"].shift(5).fillna(df_bt["c_ma60"])
        df_bt["ma60_falling"] = df_bt["c_ma60"] < (df_bt["ma60_prev5"] * 0.998)
        df_bt["is_bull"] = df_bt["close"] > df_bt["c_ma20"]
        df_bt["is_bull_m"] = df_bt["is_bull"].shift(1)
        df_bt["ma60_falling_m"] = df_bt["ma60_falling"].shift(1)
        df_bt["macro_pos_m"] = df_bt["macro_pos"].shift(1)
        
    all_events.append(df_bt)

if all_events:
    master_df = pd.concat(all_events, ignore_index=True)
    master_df = master_df.sort_values("utc_time").reset_index(drop=True)
    
    MAX_SLOTS = 10
    FEE_RATE = 0.0020
    active_positions = {}
    
    unique_times = master_df["utc_time"].unique()
    asset_trades = collections.defaultdict(list)
    asset_time_overs = collections.defaultdict(int)
    
    grouped = master_df.groupby("utc_time")
    pending_orders = collections.defaultdict(list)
    
    # 🎯 1. t봉 신호 발생
    for df in all_events:
        sym = df["symbol"].iloc[0]
        is_3x = df["is_3x"].iloc[0]
        is_2x = df["is_2x"].iloc[0]
        
        if is_3x:
            dip_rate = 0.945 - 0.020 # 🎯 -7.5%
            ...
            avg_sigma = float(np.median(df["curr_sigma"].iloc[-20:]))
            dip_rate = max(0.950, 1.0 - 2.5 * avg_sigma)
            escape_target_pnl = max(0.004, 1.5 * avg_sigma)
            hard_stop_rate = min(-0.060, -5.0 * avg_sigma)
            max_holding_bars = 48
        ...
        for i in range(1, len(df)):
            ...
            curr_p = curr_row["close"]
            prev_p = prev_row["close"]
            curr_sigma = curr_row["curr_sigma"]
            rv_threshold = curr_row["rv_threshold"]
            
            if pd.isna(rv_threshold):
                continue
                
            is_bull_m = curr_row["is_bull_m"]
            ma60_falling_m = curr_row["ma60_falling_m"]
            macro_pos_m = curr_row["macro_pos_m"]
            signal_macro_pos = curr_row.get("signal_macro_pos", np.nan)
            
            if pd.isna(is_bull_m):
                is_bull_m = curr_row["close"] > curr_row["c_ma20"]
                ma60_falling_m = False
                macro_pos_m = curr_row["macro_pos"]
                
            is_real_bear_m = (curr_row["close"] < curr_row["c_ma60"]) and ma60_falling_m
            
            if pos is None:
                is_calm = curr_sigma < rv_threshold
                touched_lower = prev_p <= prev_row["dyn_lower"]
                
                # 🎯 1x 직전봉 몸통 상단 회복
                if not (is_3x or is_2x):
                    prev_body_top = max(prev_row["open"], prev_row["close"])
                    is_bullish_bounce = (curr_p >= prev_p * 1.002) and (curr_p > curr_row["dyn_lower"]) and (curr_p > prev_body_top)
                else:
                    is_bullish_bounce = (curr_p >= prev_p * 1.002) and (curr_p > curr_row["dyn_lower"])
                    
                band_spread = (curr_row["dyn_upper"] - curr_row["dyn_lower"]) / curr_p
                has_enough_spread = band_spread >= min_band_spread
                
                if is_real_bear_m or ma60_falling_m:
                    macro_allow = False
                elif is_bull_m:
                    macro_allow = macro_pos_m <= 80.0
                else:
                    macro_allow = macro_pos_m <= macro_allow_cap

                # GC_REGIME 추가 필터:
                # 1x는 360봉 채널 55% 이하, 2x/3x는 45% 이하에서만 신규 진입.
                # 360봉 히스토리가 아직 확보되지 않아 NaN이면 진입시키지 않는다.
                gc_regime_cap = 45.0 if (is_3x or is_2x) else 55.0
                gc_regime_allow = (
                    pd.notna(signal_macro_pos)
                    and signal_macro_pos <= gc_regime_cap
                )
                ...
                if (
                    ...
                    and macro_allow
                    and gc_regime_allow
                    and has_enough_spread
                ):
                    if i + 1 < len(df):
                        next_time = df.iloc[i+1]["utc_time"]
                        _trend = "정배열(상승)" if is_bull_m else "비정배열(횡보)"
                        _macro = f"시장채널{macro_pos_m:.0f}%"
                        _signal_macro = f"종목360채널{signal_macro_pos:.0f}%"
                        reason = f"{_trend} | {_macro} | {_signal_macro} | 하단밴드반등"
                        
                        pending_orders[next_time].append({
                            "type": "ENTRY",
                            "symbol": sym,
                            "time_str": curr_row["time_str"],
                            "reason": reason
                        })
                        pos = "LONG"
                        first_entry_price = df.iloc[i+1]["open"]
                        avg_price = first_entry_price
                        holding_units = 0.5
                        holding_period = 0
                        has_taken_tp1 = False
                        tp1_pnl = 0.0
                        entry_time = df.iloc[i+1]["time_str"]
            else:
                holding_period += 1
                current_pnl = (curr_p - avg_price) / avg_price
                
                # 🎯 TP1 우선순위 처리
                will_tp1 = (not has_taken_tp1) and (curr_p >= curr_row["dyn_upper"])
                
                is_trend_exit = (has_taken_tp1 or will_tp1) and (curr_p < curr_row["mid_line"])
                is_escape_exit = (not has_taken_tp1) and (not will_tp1) and (current_pnl >= escape_target_pnl or curr_p >= curr_row["mid_line"])
                ...
                if is_trend_exit or is_escape_exit or is_spike or is_timeout or is_hard_stop:
                    if i + 1 < len(df):
                        next_time = df.iloc[i+1]["utc_time"]
                        reason = ("trend_exit" if is_trend_exit else
                                 "hard_stop" if is_hard_stop else
                        ...
                        pos = None
                        if is_timeout:
                            asset_time_overs[sym] += 1
                            
                elif will_tp1:
                    has_taken_tp1 = True
                    tp1_pnl = current_pnl
                    
                elif holding_units == 0.5 and (curr_p <= first_entry_price * dip_rate):
                    if i + 1 < len(df):
                        next_time = df.iloc[i+1]["utc_time"]
                        _dip_pct = (curr_p / first_entry_price - 1.0) * 100.0
                        reason = f"눌림{_dip_pct:.1f}%"
                        pending_orders[next_time].append({
                            "type": "SCALE_IN",
                            "symbol": sym,
                            "time_str": curr_row["time_str"],
                        ...
                        avg_price = (first_entry_price + df.iloc[i+1]["open"]) / 2.0
                        holding_units = 1.0
                        
        if pos == "LONG":
            last_time = df.iloc[-1]["utc_time"]
            pending_orders[last_time].append({
                "type": "FORCE_CLOSE",
                "symbol": sym,
                "reason": "force_close",
                "will_tp1": False
    ...
    # 🎯 2. t+1봉 시가(Open)로 체결 (시간순 시뮬레이션)
    used_slots = 0
    MAX_SLOTS_FRACTION = 1.0 / MAX_SLOTS
    cash = 1.0
    portfolio_history = []
    
    for t in sorted(unique_times):
        row_group = grouped.get_group(t)
        price_map = dict(zip(row_group["symbol"], row_group["open"]))
        close_map = dict(zip(row_group["symbol"], row_group["close"]))
        time_str = row_group["time_str"].iloc[0]
        
        orders = pending_orders.get(t, [])
        for order in [o for o in orders if o["type"] in ["EXIT", "FORCE_CLOSE"]]:
            sym = order["symbol"]
            if sym in active_positions:
                pos = active_positions[sym]
                exit_p = price_map.get(sym)
                if exit_p is None:
                    exit_p = close_map.get(sym, pos["avg_price"])
                    
                if order["will_tp1"] and not pos["has_taken_tp1"]:
                    pos["tp1_pnl"] = (exit_p - pos["avg_price"]) / pos["avg_price"]
                    pos["has_taken_tp1"] = True
                    
                curr_pnl = (exit_p - pos["avg_price"]) / pos["avg_price"]
                
                if pos["has_taken_tp1"]:
                    gross_pnl = (pos["tp1_pnl"] * 0.5) + (curr_pnl * 0.5)
                else:
                    gross_pnl = curr_pnl
                    
                net_pnl = gross_pnl - FEE_RATE
                
                invested = MAX_SLOTS_FRACTION * pos["units"]
                returned = invested * (1.0 + net_pnl)
                cash += returned
                
                used_slots -= 1
                
                asset_trades[sym].append({
                    "entry_time": pos["entry_time"],
                    "entry_price": pos["first_price"],
                    "scale_in_time": pos.get("scale_in_time", ""),
                    "scale_in_price": pos.get("scale_in_price", 0.0),
                    "exit_time": time_str,
                    "exit_price": exit_p,
                    "pnl": net_pnl,
                    "scale_in": pos["units"] == 1.0,
                    "is_escape": order["reason"] == "escape_exit",
                    "is_hard_stop": order["reason"] == "hard_stop",
                    "entry_reason": pos["entry_reason"],
                    "scale_in_reason": pos.get("scale_in_reason", ""),
                    "exit_reason": order["reason"]
                })
                del active_positions[sym]
                
        for order in [o for o in orders if o["type"] == "ENTRY"]:
            if used_slots < MAX_SLOTS:
                sym = order["symbol"]
                if sym not in active_positions:
                    entry_p = price_map.get(sym)
                    if entry_p:
                        invested = MAX_SLOTS_FRACTION * 0.5
                        if cash >= invested:
                            cash -= invested
        ...
        for order in [o for o in orders if o["type"] == "SCALE_IN"]:
            sym = order["symbol"]
            if sym in active_positions and active_positions[sym]["units"] == 0.5:
                scale_p = price_map.get(sym)
                if scale_p:
                    invested = MAX_SLOTS_FRACTION * 0.5
                    if cash >= invested:
                        cash -= invested
                        active_positions[sym]["scale_in_time"] = time_str
                        active_positions[sym]["scale_in_price"] = scale_p
                        active_positions[sym]["scale_in_reason"] = order["reason"]
                        active_positions[sym]["avg_price"] = (active_positions[sym]["first_price"] + scale_p) / 2.0
                        active_positions[sym]["units"] = 1.0
                    
        for sym in active_positions:
            active_positions[sym]["holding_bars"] += 1
            
        mtm_value = cash
        for sym, pos in active_positions.items():
            curr_p = price_map.get(sym)
            if curr_p is None:
                curr_p = close_map.get(sym, pos["avg_price"])
            
            curr_pnl = (curr_p - pos["avg_price"]) / pos["avg_price"] - FEE_RATE
            if pos["has_taken_tp1"]:
                gross_pnl = (pos["tp1_pnl"] * 0.5) + (curr_pnl * 0.5)
            else:
                gross_pnl = curr_pnl
            ...
            invested = MAX_SLOTS_FRACTION * pos["units"]
            pos_value = invested * (1.0 + gross_pnl)
            mtm_value += pos_value
            
        portfolio_history.append({"time": t, "equity": mtm_value})
        
    st.session_state["portfolio_history"] = portfolio_history

    # 🎯 3. 결과 패킹
    for res in all_calculated:
        sym = res["target_info"]["symbol"]
        res["trade_log"] = asset_trades.get(sym, [])
        res["trade_returns"] = [t["pnl"] for t in res["trade_log"]]
        res["time_over_count"] = asset_time_overs.get(sym, 0)
        
        if sym in active_positions:
            pos = active_positions[sym]
            last_df = next(df for df in all_events if df["symbol"].iloc[0] == sym)
            curr_close = last_df.iloc[-1]["close"]
            curr_pnl = (curr_close - pos["avg_price"]) / pos["avg_price"] - FEE_RATE
            res["current_holding"] = {
                "entry_time": pos["entry_time"],
                "entry_price": pos["first_price"],
                "avg_price": pos["avg_price"],
                "current_price": curr_close,
                "holding_units": pos["units"],
                "holding_bars": pos["holding_bars"],
                "scale_in_time": pos.get("scale_in_time", ""),
                "scale_in_price": pos.get("scale_in_price", 0.0),
                "unrealized_pnl": float(curr_pnl),
                "entry_reason": pos["entry_reason"],
                "scale_in_reason": pos.get("scale_in_reason", "")
            }
        else:
            res["current_holding"] = None
            
        if res["trade_log"] and res["df_bt"] is not None and not res["df_bt"].empty:
            latest_trade_date = str(res["df_bt"]["time_str"].iloc[-1]).split()[0]
            res["today_trades"] = [
                t["pnl"] for t in res["trade_log"]
...
styled_df_kr = df_kr
if not df_kr.empty:
    if hasattr(df_kr.style, "map"):
        styled_df_kr = df_kr.style.map(style_rank, subset=["순위"])
    else:
        styled_df_kr = df_kr.style.applymap(style_rank, subset=["순위"])

styled_df_us = df_us
if not df_us.empty:
    if hasattr(df_us.style, "map"):
        styled_df_us = df_us.style.map(style_rank, subset=["순위"])
    else:
        styled_df_us = df_us.style.applymap(style_rank, subset=["순위"])
...
with st.expander("🔬 [통계 및 실전 검증] FPCA 변동성 예측 모형 & 실전 동적 가이드 신뢰도", expanded=True):
    ...
    with tab_simulation:
        ...
        if N_total >= 30:
            ...
            # 🎯 MTM Equity Curve & MDD Display
            if "portfolio_history" in st.session_state and len(st.session_state["portfolio_history"]) > 0:
                df_eq = pd.DataFrame(st.session_state["portfolio_history"])
                # 타임존 정보 제거하여 Plotly shared_xaxes 호환성 확보 (tz-naive UTC)
                df_eq["time"] = pd.to_datetime(df_eq["time"]).dt.tz_localize(None)
                df_eq.set_index("time", inplace=True)
                
                # 자산가치 -> 수익률(%) 변환으로 변화량 증폭
                df_eq["return_pct"] = (df_eq["equity"] - 1.0) * 100.0
                
                roll_max = df_eq["equity"].cummax()
                drawdown = (df_eq["equity"] / roll_max - 1.0) * 100.0
                mdd = float(drawdown.min())
                final_return = float(df_eq["equity"].iloc[-1] - 1.0)
                
                e1, e2, e3 = st.columns(3)
                e1.metric("💰 MTM 누적 수익률", f"{final_return*100:+.2f}%", delta=f"최종 자산 {df_eq['equity'].iloc[-1]:.4f}")
                e2.metric("📉 최대 낙폭 (MDD)", f"{mdd:.2f}%", delta_color="inverse")
                e3.metric("🔄 총 포지션 체결/청산", f"{N_total:,}회", delta=f"승률 {win_rate_total:.1f}%")
                
                import plotly.graph_objects as go
                from plotly.subplots import make_subplots
                
                ret_vals = df_eq["return_pct"].values

                # ── 국장/미장 평균 등락률 계산
                def compute_avg_return_series(dfs):
                    if not dfs:
                        return pd.Series(dtype=float)
                    all_ret = []
                    for df in dfs:
                        d = df.sort_values("utc_time").copy()
                        first_v = d["close"].iloc[0]
                        if first_v > 0:
                            ret_s = pd.Series(
                                ((d["close"].values / first_v) - 1.0) * 100.0,
                                index=pd.to_datetime(d["utc_time"]).dt.tz_localize(None)
                            )
                            all_ret.append(ret_s)
                    if not all_ret:
                        return pd.Series(dtype=float)
                    combined = pd.concat(all_ret, axis=1)
                    return combined.mean(axis=1).sort_index()

                kr_dfs = [df for df in all_events if df["is_kr"].iloc[0]]
                us_dfs = [df for df in all_events if not df["is_kr"].iloc[0]]
                kr_ret = compute_avg_return_series(kr_dfs)
                us_ret = compute_avg_return_series(us_dfs)
                
                fig = go.Figure()
                
                # ── 포트폴리오 누적 수익률 %
                pos_fill = [max(v, 0) for v in ret_vals]
                neg_fill = [min(v, 0) for v in ret_vals]
                
                fig.add_trace(
                    go.Scatter(x=df_eq.index, y=pos_fill, mode="none",
                        fill="tozeroy", fillcolor="rgba(34,197,94,0.15)",
                        showlegend=False, hoverinfo="skip")
                )
                fig.add_trace(
                    go.Scatter(x=df_eq.index, y=neg_fill, mode="none",
                        fill="tozeroy", fillcolor="rgba(239,68,68,0.15)",
                        showlegend=False, hoverinfo="skip")
                )
                
                line_color = "#22c55e" if final_return >= 0 else "#ef4444"
                fig.add_trace(
                    go.Scatter(
                        x=df_eq.index, y=ret_vals,
                        mode="lines", name="📈 포트폴리오",
                        line=dict(color=line_color, width=2.5),
                        hovertemplate="<b>%{x|%m/%d %H:%M}</b><br>포트폴리오: <b>%{y:+.3f}%</b><extra></extra>"
                    )
                )
                ...
                if not kr_ret.empty:
                    fig.add_trace(
                        go.Scatter(
                            x=kr_ret.index, y=kr_ret.values,
                            mode="lines", name="🌏 국장 평균",
                            line=dict(color="#f59e0b", width=1.8, dash="solid"),
                            hovertemplate="<b>%{x|%m/%d %H:%M}</b><br>국장: <b>%{y:+.3f}%</b><extra></extra>"
                        )
                    )
                # ── 미장 평균 등락률
                if not us_ret.empty:
                    fig.add_trace(
                        go.Scatter(
                            x=us_ret.index, y=us_ret.values,
                            mode="lines", name="🇺🇸 미장 평균",
                            line=dict(color="#818cf8", width=1.8, dash="solid"),
                            hovertemplate="<b>%{x|%m/%d %H:%M}</b><br>미장: <b>%{y:+.3f}%</b><extra></extra>"
                        )
                    )
                
                fig.add_hline(y=0, line_dash="dot",
                    line_color="rgba(148,163,184,0.6)", line_width=1.5)
                
                _grid = dict(showgrid=True, gridcolor="rgba(148,163,184,0.1)", zeroline=True,
                             zerolinecolor="rgba(148,163,184,0.3)")
                fig.update_layout(
                    title=dict(
                        text="📈 포트폴리오 누적 수익률 vs 시장 평균 (%)",
                    ...
                    hovermode="x unified",
                    xaxis=dict(**_grid),
                    yaxis=dict(**_grid, ticksuffix="%", tickformat=".2f",
                               title=dict(text="수익률 / 등락률 (%)", font=dict(size=11)), automargin=True),
                )
            ...
            st.markdown(
                ...
                <div style="font-size: 13px; color: #1e293b; line-height: 1.6; background-color: #f8fafc; padding: 12px 16px; border-radius: 8px; border: 1px solid #e2e8f0; margin-top: 10px;">
                    ...
                    - 총 <b>{N_total:,}회</b> 체결 동안 승률 <b>{win_rate_total:.1f}%</b>, 건당 초과수익 <b>{actual_mean_total*100:+.2f}%</b>를 기록하며 실전 가이드로서의 유효성을 보여줘.<br>
                    - (※ 미실현 손익을 포함한 MTM 자산 성장 곡선과 MDD가 성공적으로 반영되었어!)
                </div>
                """,
                unsafe_allow_html=True
            )
    ...
    with tab_today:
        ...
        for d in full_ranked:
            ...
            if holding is not None:
                ...
                holding_rows.append({
                    ...
                    "1차 진입 이유": holding.get("entry_reason", "-"),
                    "2차 매수 이유": holding.get("scale_in_reason", "-") if holding["holding_units"] == 1.0 else "-",
        ...
        with st.expander(f"💼 현재 보유 중인 종목 ({num_holdings}개 진행 중)", expanded=(num_holdings > 0)):
            if num_holdings > 0:
                ...
                show_cols = ["종목명", "진입 시점", "보유 비중", "평단가", "현재가", "평가 수익률", "평가 손익금", "보유 시간", "1차 진입 이유", "2차 매수 이유"]
        ...
        for d in full_ranked:
            ...
            if len(t_trades) > 0:
                ...
                tot_pnl = float(np.sum(t_trades_arr))
        ...
        if len(today_data) > 0:
            ...
            today_total_pnl = float(np.sum(all_today_arr))  # 🎯 NameError 원인이었던 합산 수익률 변수 복구

            # 인버스 키워드 (계산에서 제외)
            _inverse_kw = ["인버스", "곱버스", "SQQQ", "SOXS", "SPXU", "TZA", "TSLS", "FNGD", "LABD"]
            def _is_inverse(d):
                name = d.get("asset_name", "")
                return any(kw in name for kw in _inverse_kw)

            # 국장 / 미장 평균 등락률 (인버스 제외)
            kr_returns = [d.get("recent_return", 0.0) for d in full_ranked
                          if d.get("target_info", {}).get("is_kr", False) and not _is_inverse(d)]
            us_returns = [d.get("recent_return", 0.0) for d in full_ranked
                          if not d.get("target_info", {}).get("is_kr", True) and not _is_inverse(d)]
            kr_avg = float(np.mean(kr_returns)) if kr_returns else 0.0
            us_avg = float(np.mean(us_returns)) if us_returns else 0.0

            def _market_label(v):
                if v >= 0.005: return "강세장"
                if v <= -0.005: return "약세장"
                return "횡보장"
            ...
            t4.metric(
                "🇰🇷 국장 평균 등락률",
                f"{kr_avg * 100:+.2f}%",
                delta=_market_label(kr_avg),
                delta_color="normal" if kr_avg >= 0 else "inverse"
            )
            t5.metric(
                "🇺🇸 미장 평균 등락률",
                f"{us_avg * 100:+.2f}%",
                delta=_market_label(us_avg),
                delta_color="normal" if us_avg >= 0 else "inverse"
            )
            t6.metric("최다 체결 종목", max(today_data, key=lambda x: x["_cnt"])["종목명"])

            df_today = pd.DataFrame(today_data).sort_values(by="_sort_tot", ascending=False)
            display_cols = ["종목명", "체결 횟수", "승률", "건당 평균 수익률", "합산 수익률"]
            ...
            st.info("💡 오늘 당일 청산(체결 완료)된 거래 내역이 아직 없어. (장중 시그널 진행 또는 대기 중)")
# ------------------------------------------------------------------------------
# 8-2. 상세 종목 탭 렌더링
# ------------------------------------------------------------------------------
st.markdown("---")
...
for tab, data in zip(tabs, display_targets):
    with tab:
        ...
        with col_short:
            ...
            target_tz = pytz.timezone(target_info.get("tz", "Asia/Seoul"))
            now_target = datetime.now(target_tz)

            if not is_open:
                close_h, close_m = (15, 30) if is_kr_stock else (16, 0)
                base_dt = now_target.replace(
                    hour=close_h, minute=close_m, second=0, microsecond=0
                )
                if base_dt > now_target:
                ...
                next_open_str = "익일 09:30"
                next_open_plus_str = "익일 10:00"
                future_labels = [
                    last_time_label,
                    f"{next_open_str} (예측)",
                ...
                base_dt = now_target.replace(
                    minute=(now_target.minute // 5) * 5, second=0, microsecond=0
                )
                time_labels = [
                    (base_dt - timedelta(minutes=(23 - i) * 5)).strftime("%H:%M")
                    for i in range(24)
            ...
            if macro is not None:
                ...
                if trade_log:
                    ...
                    fig_long.add_trace(go.Scatter(
                        ...
                        customdata=[t.get("entry_reason", "?") for t in trade_log],
                        hovertemplate="<b>[1차 매수]</b> %{y:,.2f}<br>진입 이유: <b>%{customdata}</b><br>일시: %{x}<extra></extra>"
                    ))
                    ...
                    if loss_trades:
                        fig_long.add_trace(go.Scatter(
                            ...
                            customdata=[[t["pnl"] * 100, t.get("exit_reason", "?"), "Y" if t.get("scale_in") else "N"] for t in loss_trades],
                            hovertemplate="<b>[손절]</b> %{y:,.2f} (%{customdata[0]:.2f}%)<br>원인: <b>%{customdata[1]}</b><br>2차매수: %{customdata[2]}<br>일시: %{x}<extra></extra>"
                        ))

                    # 🔴 현재 보유 중인 포지션 진입 마커 (★ 별)
                    ch = data.get("current_holding")
                    if ch is not None:
                        hold_label = "100% (2차)" if ch["holding_units"] == 1.0 else "50% (1차)"
                        pnl_sign = "+" if ch["unrealized_pnl"] >= 0 else ""
                        fig_long.add_trace(go.Scatter(
                            x=[ch["entry_time"]],
                            y=[ch["entry_price"]],
                            mode="markers+text",
                            name="보유 중 (미청산)",
                            marker=dict(symbol="star", size=16, color="#a855f7", line=dict(width=1.5, color="#ffffff")),
                            text=["보유중"],
                            textposition="top center",
                            textfont=dict(size=9, color="#7c3aed"),
                            customdata=[[ch["entry_price"], pnl_sign + f"{ch['unrealized_pnl']*100:.2f}%",
                                         hold_label, ch.get("entry_reason", "?"), ch["holding_bars"]]],
                            hovertemplate=(
                                "<b>⭐ [보유 중 - 미청산]</b><br>"
                                "진입가: <b>%{customdata[0]:,.2f}</b><br>"
                                "평가수익률: <b>%{customdata[1]}</b><br>"
                                "비중: %{customdata[2]}<br>"
                                "진입 이유: <b>%{customdata[3]}</b><br>"
                                "보유 시간: %{customdata[4]}봉<br>"
                                "진입 시점: %{x}<extra></extra>"
                            )
                        ))
                        # 2차 매수도 보유 중이면 마커 추가
                        if ch["holding_units"] == 1.0 and ch.get("scale_in_time"):
                            fig_long.add_trace(go.Scatter(
                                x=[ch["scale_in_time"]],
                                y=[ch["scale_in_price"]],
                                mode="markers",
                                name="보유 중 2차 매수",
                                marker=dict(symbol="star", size=14, color="#f59e0b", line=dict(width=1.5, color="#ffffff")),
                                hovertemplate="<b>⭐ [보유 중 - 2차 매수]</b><br>진입가: %{y:,.2f}<br>일시: %{x}<extra></extra>"
                            ))
